...
uint Get_Valid(uint _s,uint _E){return Cache.cache[_s][_E][0];}
uint Get_Tag(uint _s,  uint _E){return Cache.cache[_s][_E][1];}
uint Get_Lru(uint _s,  uint _E){return Cache.cache[_s][_E][2];}
void Get_Cache(){
	Cache.cache=(uint ***)malloc(Cache.s*sizeof(uint **));
	FOR(i,0,Cache.s)Cache.cache[i]=(uint **)malloc(Cache.E*sizeof(uint *));
	FOR(i,0,Cache.s)FOR(j,0,Cache.E)Cache.cache[i][j]=(uint *)malloc(3*sizeof(uint));
...
void Puts_Set(uint _s){
	FOR(i,0,Cache.E)
		printf("%lu %lx %lu\n",Cache.cache[_s][i][0],Cache.cache[_s][i][1],Cache.cache[_s][i][2]);
}
void Puts_Cache(){
